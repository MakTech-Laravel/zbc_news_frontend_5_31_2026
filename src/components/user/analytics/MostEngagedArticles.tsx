...
import type { EngagedArticleItem } from "@/types/readingAnalytics";

type MostEngagedArticlesProps = {
  articles: EngagedArticleItem[];
};

export function MostEngagedArticles({ articles }: MostEngagedArticlesProps) {
  return (
    <UserDashboardCard className="rounded-[14px] border-black/10">
      <div className="space-y-1 px-6 pb-2 pt-6">
        <h3 className="font-inter text-base font-medium text-ink-heading">
      ...
      {articles.length > 0 ? (
        <ul className="space-y-4 px-6 pb-6">
          {articles.map((article) => (
            <li key={`${article.id}-${article.rank}`} className="space-y-2">
        ...
        <p className="px-6 pb-6 text-sm text-ink-muted">No engaged articles yet.</p>
      )}
    </UserDashboardCard>
  );
}
