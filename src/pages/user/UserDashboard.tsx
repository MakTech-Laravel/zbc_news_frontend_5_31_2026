    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-6 xl:grid-cols-[minmax(0,1fr)_min(100%,503px)]">
        <div className="min-w-0 space-y-4 sm:space-y-6">
          <UserFeaturedStory story={featuredStory} />
          <UserPersonalizedFeed feeds={loading ? undefined : data?.feeds} />
        </div>
        ...
        <aside className="space-y-4 sm:space-y-6 xl:sticky xl:top-4 xl:max-h-[calc(100dvh-var(--user-header-height,68px)-2rem)] xl:overflow-y-auto xl:scrollbar-hide xl:self-start">
          <UserContinueReading items={loading ? undefined : data?.continue_reading} />
          <UserTrendingTopics topics={loading ? undefined : data?.trending_topics} />
          <UserThisWeek stats={loading ? undefined : data?.this_week} />
        </aside>
      </div>
    </div>
  );
