...
export type BreakingNewsSidebarData = {
...
export type BreakingNewsData = {
  ...
  sidebar: BreakingNewsSidebarData;
};
...
function buildBreakingNewsData(
  feeds: TagArticleFeeds,
...
export function useBreakingNews(tagSlug: string = BREAKING_NEWS_TAG_SLUG) {
  const [data, setData] = React.useState<BreakingNewsData>(() =>
    buildBreakingNewsData(EMPTY_FEEDS, [], null),
  );
    ...
    Promise.all([
      ...
      .then(([feeds, latestRows, trendingRows, analytics]) => {
        ...
        const articlesThisWeek = Number(
          analytics?.metrics.find((metric) => metric.id === "articles")?.value ?? feeds.latest.length,
        );

        setData(
          buildBreakingNewsData(feeds, trendingRows, featuredStory, {
            articlesThisWeek: Number.isNaN(articlesThisWeek) ? feeds.latest.length : articlesThisWeek,
      ...
      .catch((err: unknown) => {
        if (cancelled) return;
        setData(buildBreakingNewsData(EMPTY_FEEDS, [], null));
