...
> **Confirmed (Phase 3):** JSON-LD is emitted only where §4 specifies it — `NewsArticle` on article pages and `Organization`+`WebSite` on home. **Empty `json_ld[]` on category, tag, and author pages is intentional**, not a gap: those page types were never assigned structured data here, and the sole category/article option (`BreadcrumbList`) was explicitly deferred as optional. If Breadcrumb/CollectionPage structured data is later wanted for those types, it's an additive follow-on.

**Social image fallback (defensive — decided Phase 3):** OG/Twitter image resolves per page type, then falls back to `site_settings.site_logo`, then to a configurable default asset `config('app.og_default_image')` (env `OG_DEFAULT_IMAGE`). Only when **all** of those are empty is `og:image`/`twitter:image` omitted and a plain `summary` Twitter card emitted — a valid, now-explicit state rather than a silent downgrade. `SiteSettingsSeeder` seeds a real `site_logo` (and `meta_title`/`description`/`keywords`) so fresh installs and CI always produce a `summary_large_image` card. (The earlier missing-image symptom was a seeder gap: the Cloudinary logo seen in the first verification came from a manual admin edit on the old dev DB, not seed data.)
